...
var makeMaze = function (x) {
  const arr = [...x];
  var total = x.length;
  var n = Math.sqrt(total);
  //code taken from https://stackoverflow.com/questions/22464605/convert-a-1d-array-to-2d-array
  const newArr = [];
...
var findPath = function (maze) {
    ...
    while (q.length > 0) {
      ...
      for (var i = 0; i < NEIGHBOR_DIRS.length; i++) {
        ...
        if (
          ...
          maze[new_x][new_y] != 1 &&
          curr_distance + 1 < distance[new_x][new_y]
        ) {
    ...
    while (openList.length > 0) {
      ...
      for (var i = 0; i < NEIGHBOR_DIRS.length; i++) {
        ...
        if (
          is_valid(new_x, new_y) &&
          maze[new_x][new_y] != 1 &&
          !closeList.has([new_x, new_y])
        ) {
          var d = curr_distance + 1;
          if (d < distance[new_x][new_y]) {
            var expected_distance =
              Math.abs(new_x - START[0]) + Math.abs(new_y - START[1]);
            distance[new_x][new_y] = d;
            openList.push([
              [new_x, new_y],
              expected_distance + distance[new_x][new_y],
            ]);
          }
        }
      }
  ...
  return { dist: distance, succeed: distance[START[0]][START[1]] != Infinity && !maze[END[0]][END[1]]};
};
      ...
      for (var i = 0; i < NEIGHBOR_DIRS.length; i++) {
        ...
        if (
          ...
          if (d < distance_AStar[new_x][new_y]) {
            ...
            openList.push([
              [new_x, new_y],
              expected_distance + distance_AStar[new_x][new_y],
            ]);
          }
        }
      }
    ...
    while (curr[0] != END[0] || curr[1] != END[1]) {
      ...
      if (!isAStar) {
        ...
        if (!isRandom && bestPath.length != 0) {
          ...
          //pick the more likely possible path
          ...
          for (var i = 0; i < NEIGHBOR_DIRS.length; i++) {
            ...
            var new_y = curr[1] + NEIGHBOR_DIRS[i][1];
            var expected_distance =
              Math.abs(new_x - END[0]) + Math.abs(new_y - END[1]);
            if (is_valid(new_x, new_y) && maze[new_x][new_y] != 1) {
              if (
                min_expected_distance >
                distance[new_x][new_y] + expected_distance
              ) {
                next = i;
                min_expected_distance =
                  distance[new_x][new_y] + expected_distance;
      ...
      curr = [
        curr[0] + NEIGHBOR_DIRS[next][0],
        curr[1] + NEIGHBOR_DIRS[next][1],
      ];
    }
    return path;
  }
...
var calculateScore = function(input){
    MAZE_SIZE = 3
    var prob_sum = 0
    var x = Array(MAZE_SIZE).fill().map(() => Array(MAZE_SIZE).fill(0));
   // console.log(input)
    for(var idx in input)
    {
       // console.log(input[i].value)
        var val = input[idx].value
        var score = Math.exp(input[idx].score)
       // val = val * score
        var  val2 = val.map(x => (x.map(y => y * score)));
    //    console.log(val2)
